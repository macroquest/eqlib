...
enum class GameFeature
{
	...
	DefinedMax_,             // private for emu implementation. do not use this.
	Max = Ornament + 1,
	First = 0,
	Invalid = -1,
};
...
enum class MembershipLevel : int8_t
{
	...
	// These don't exist in ROF2
	AllAccess = Gold,
	LifetimeAllAccess = Gold,
};
...
{
private:
	//  Marked private with slightly different name. See comment below.
	virtual int GetGameFeature_(GameFeature feature) const = 0;

public:
	int GetGameFeature(GameFeature feature) const
	{
		// We define values that don't exist for compatibility with live. In order to support
		// this, we need to filter the input and handle the values that don't exist.
		if (feature >= GameFeature::First && feature < GameFeature::Max)
			return GetGameFeature_(feature);

		if (feature >= GameFeature::Max && feature < GameFeature::DefinedMax_)
		{
			const RestrictionInfo& restrictionInfo = FreeToPlayClient::RestrictionInfo[static_cast<int>(feature)];

			if (restrictionInfo.boolean)
				return 0;  // "false"
...
private:
	virtual int GetGameFeature_(GameFeature) const override { return 0; }
public:
