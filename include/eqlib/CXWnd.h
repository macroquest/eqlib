...
public:
	...
	void SetClickThrough(bool bValue) { UNUSED(bValue); }
	...
	void SetDisabledBackground(COLORREF Value) { UNUSED(Value); }
	COLORREF GetDisabledBackground() const { return 0; }
	...
	void SetData(int64_t Value) { Data = static_cast<int32_t>(Value); }
	int64_t GetData() const { return Data; }

	void SetShowClickThroughMenuItem(bool bValue) { UNUSED(bValue); }
	bool GetShowClickThroughMenuItem() const { return true; }
...
// @end: CXWnd Members
	...
	bool& get_ClickThrough() { static bool value; return value; }
	__declspec(property(get = get_ClickThrough)) bool& bClickThroughMenuItemStatus;
	__declspec(property(get = get_ClickThrough)) bool& bClickThroughToBackground;
	__declspec(property(get = get_ClickThrough)) bool& bShowClickThroughMenuItem;

	bool& get_Border() { static bool Border; return Border; }
	__declspec(property(get = get_Border)) bool& bEnableShowBorder;
	__declspec(property(get = get_Border)) bool& bShowBorder;
	DEPRECATE("Use bEnableShowBorder instead of bBorder")
