...
/*0x0194*/ unsigned int             CombatSkillUsed[0x2];
/*0x019c*/ int                      EnduranceCurrent;
/*0x01a0*/ bool                     bShowHelm;
/*0x01a8*/ int64_t                  HPCurrent;
/*0x01b0*/ int                      LastCastNum;
/*0x01b4*/ int                      SecondaryTintIndex;
/*0x01b8*/ bool                     bSummoned;                    // by a PC or by anyone? need to investigate
/*0x01bc*/ int                      PetID;
/*0x01c0*/ uint8_t                  GMRank;
/*0x01c1*/ char                     DragNames[0x2][0x40];
/*0x0241*/ char                     Handle[0x20];
/*0x0261*/ uint8_t                  HoldingAnimation;             // todo: create enum for this byte. Holding: Nothing=0 A RightHand Weapon=1 A Shield=2 Dual Wielding Two Weapons=3 A Spear=4 A LeftHand Weapon=5 A Two Handed Weapon=6 A bow=7
/*0x0262*/ uint8_t                  Light;
/*0x0264*/ unsigned int             SitStartTime;
/*0x0268*/ char                     DraggingPlayer[0x40];
/*0x02a8*/ uint8_t                  FishingEvent;
/*0x02ac*/ int                      Dontknowyet2;
/*0x02b0*/ int                      RealEstateItemId;
/*0x02b4*/ unsigned int             CorpseDragCount;
/*0x02b8*/ bool                     PvPFlag;
/*0x02bc*/ float                    GetMeleeRangeVar1;            // used by GetMeleeRange (fScaleFactor)
/*0x02c0*/ int                      Buyer;
/*0x02c4*/ bool                     bOfflineMode;
/*0x02c8*/ int                      AFK;
/*0x02cc*/ int                      Anon;                         // found in EQPlayer__SetNameSpriteTint_x
/*0x02d0*/ char                     LoginRelated[0x20];
/*0x02f0*/ bool                     bBetaBuffed;
/*0x02f4*/ unsigned int             LastRefresh;                  // last 18 sec tick
/*0x02f8*/ uint8_t                  TitleVisible;
/*0x02fc*/ float                    ViewHeight;
/*0x0300*/ int                      Deity;
/*0x0304*/ uint8_t                  LastAttack;
/*0x0305*/ uint8_t                  CharClass;                    // Im pretty sure this isnt working
/*0x0306*/ char                     GM;
/*0x0307*/ uint8_t                  IntimidateCount;
/*0x0308*/ char                     Title[0x80];
/*0x0388*/ int                      WarCry;
/*0x038c*/ unsigned int             LastResendAddPlayerPacket;
/*0x0390*/ int                      SomethingElse;
/*0x0394*/ bool                     bSwitchMoved;                 // true when a door moves near the spawn
/*0x0395*/ EqItemGuid               realEstateItemGuid;
/*0x03a8*/ unsigned int             StunTimer;
/*0x03ac*/ unsigned int             FishingETA;
/*0x03b0*/ uint8_t                  FindBits;
/*0x03b1*/ uint8_t                  Type2;
/*0x03b4*/ int                      ManaMax;
/*0x03b8*/ unsigned int             NextIntimidateTime;
/*0x03bc*/ int                      AARank;                       // this really is the title id
/*0x03c0*/ int                      AltAttack;
/*0x03c4*/ unsigned int             RespawnTimer;                 // its actually TimeOfDeath...TimeStamp of when RespawnWnd will close - 0 when you're alive
/*0x03c8*/ unsigned int             LastTrapDamageTime;
/*0x03cc*/ bool                     bTempPet;
/*0x03d0*/ unsigned int             LastRangedUsedTime;
/*0x03d4*/ unsigned int             Zone;
/*0x03d8*/ int64_t                  HPMax;
/*0x03e0*/ uint8_t                  Level;
/*0x03e4*/ float                    MerchantGreed;
/*0x03e8*/ void*                    pTouchingSwitch;              // need to get that in here : struct _EQSWITCH
/*0x03ec*/ unsigned int             LastSecondaryUseTime;
/*0x03f0*/ int                      Trader;                       // found in CEverQuest__RightClickedOnPlayer_x
/*0x03f4*/ uint8_t                  IsPassenger;                  // if u are on a boat or airship or whatever
/*0x03f8*/ int                      berserker;
/*0x03fc*/ int                      DontKnowYet;
/*0x0400*/ unsigned int             LastPrimaryUseTime;
/*0x0404*/ int                      SomeData[0x2];
/*0x040c*/ bool                     bStationary;
/*0x040d*/ bool                     Sneak;
/*0x040e*/ bool                     Mercenary;
/*0x0410*/ float                    CameraOffset;
/*0x0414*/ bool                     Linkdead;
/*0x0418*/ unsigned int             CombatSkillTicks[0x2];
/*0x0420*/ int                      IsAttacking;                  // need to investigate
/*0x0424*/ bool                     bAlwaysShowAura;
/*0x0428*/ unsigned int             Meditating;
/*0x042c*/ unsigned int             LastTimeStoodStill;
/*0x0430*/ unsigned int             SpellCooldownETA;
/*0x0434*/ float                    MissileRangeToTarget;
/*0x0438*/ int                      RealEstateID;
/*0x043c*/ bool                     bAnimationOnPop;
/*0x043d*/ uint8_t                  Blind;
/*0x043e*/ uint8_t                  InPvPArea;                    // are we in a PvP area?
/*0x0440*/ void*                    pRaceGenderInfo;              // todo this is not a void* so... map the CRaceGenderInfo class
/*0x0444*/ unsigned int             MinuteTimer;
/*0x0448*/ CharacterZoneClient*     pCharacter;
/*0x044c*/ int                      CurrIOState;
/*0x0450*/ int                      NpcTintIndex;
/*0x0454*/ CPhysicsInfo             LastCollision;
/*0x0484*/ bool                     LFG;
/*0x0488*/ unsigned int             LastMealTime;                 // last time we eat or drank
/*0x048c*/ unsigned int             SpellGemETA[0xf];             // InnateETA is now the last member in this array
/*0x04c8*/ unsigned int             LoginSerial;
/*0x04cc*/ float                    MyWalkSpeed;
/*0x04d0*/ int                      Stuff;
/*0x04d4*/ unsigned int             LastCastTime;
/*0x04d8*/ unsigned int             TimeStamp;                    // updates all the time including when on a mount
/*0x04dc*/ float                    RunSpeed;                     // 0.70 on runspeed 5...
/*0x04e0*/ uint8_t                  StandState;
/*0x04e4*/ float                    BearingToTarget;
/*0x04e8*/ unsigned int**           ppUDP;                        // UdpLibrary::UdpConnection
/*0x04ec*/ unsigned int             MasterID;
/*0x04f0*/ SPAWNINFO*               pViewPlayer;
/*0x04f4*/ LaunchSpellData          CastingData;                  // size: 0x58
/*0x054c*/ char                     Suffix[0x80];
/*0x05cc*/ uint8_t                  FD;
/*0x05cd*/ bool                     bBuffTimersOnHold;            // no you can't control this client side so unless you want to get banned leave it alone
/*0x05d0*/ unsigned int             EnduranceMax;
/*0x05d4*/ int                      ManaCurrent;
/*0x05d8*/ uint8_t                  HmmWhat;
/*0x05dc*/ int                      PrimaryTintIndex;
/*0x05e0*/ unsigned int             LastTick;
/*0x05e8*/ int64_t                  GuildID;
/*0x05f0*/ int                      DoSpecialMelee;
/*0x05f4*/ float                    AnimationSpeedRelated;
/*0x05f8*/ int                      HideMode;
/*0x05fc*/ float                    FallingStartZ;
/*0x0600*/ bool                     bAttackRelated;
/*0x0604*/ unsigned int             ACounter;                     // not sure what this one is for (m_uHibernatingCount)
/*0x0608*/ unsigned int             NextSwim;
...
// @sizeof(PlayerClient) == 0x1EA0 :: 2021-10-14 (live) @ 0x69D620
