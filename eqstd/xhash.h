...
namespace eqstd
{
	...
	protected:
		...
		void _Forced_rehash(size_type _Buckets) {
			...
			const unsigned long _Max_storage_buckets_log2 = std::_Floor_of_log_2(static_cast<size_t>(_Vec.max_size() >> 1));
			...
			_Buckets = static_cast<size_type>(1) << std::_Ceiling_of_log_2(static_cast<size_t>(_Buckets));
